...
const CYBER_THEME_FILTER = `
  (
    ...
    OR THEMES LIKE '%DISINFORMATION%'
    OR THEMES LIKE '%PROPAGANDA%'
...
export async function getCyberTrends(): Promise<DayTrend[]> {
  ...
  const query = `
    SELECT
      ...
      COUNTIF(THEMES LIKE '%DISINFORMATION%' OR THEMES LIKE '%PROPAGANDA%' OR THEMES LIKE '%TAX_FNCACT_PROPAGANDIST%') AS info_ops,
